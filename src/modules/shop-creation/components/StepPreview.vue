    ...
    <div class="bg-white rounded-[3rem] border-[12px] border-gray-900 shadow-2xl overflow-hidden max-w-sm mx-auto relative">
      ...
      <div class="bg-primary pt-10 pb-6 px-6 text-white relative">
        <div class="flex items-center gap-4">
          <div class="w-16 h-16 rounded-2xl bg-white/20 backdrop-blur-md flex items-center justify-center text-2xl border border-white/30 overflow-hidden">
      ...
      <!-- Phone Content -->
      <div class="p-3 space-y-3 h-[350px] overflow-y-auto bg-gray-50 custom-scrollbar">
        <!-- Badges Info -->
        <div class="flex gap-2">
           <div v-if="store.deliveryModes.length > 0" class="flex items-center gap-1 text-[8px] font-bold bg-secondary/10 text-primary px-2 py-1 rounded-full border border-secondary/20">
             <Truck :size="10" /> Livraison dispo
           </div>
           <div v-if="store.paymentModes.length > 0" class="flex items-center gap-1 text-[8px] font-bold bg-blue-50 text-blue-600 px-2 py-1 rounded-full border border-blue-100">
